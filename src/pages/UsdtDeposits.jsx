...
import {
    ...
    ExternalLink, Send, X, Loader2, Search, Download,
    Hash, IndianRupee, TrendingUp, DollarSign, Key, Copy, Eye, EyeOff,
} from 'lucide-react';

const USDT_GREEN = '#26a17b';
const USDT_DARK  = '#0d3d2e';

const inr = (n) =>
    new Intl.NumberFormat('en-IN', { style: 'currency', currency: 'INR', maximumFractionDigits: 2 }).format(Number(n) || 0);

const usdtFmt = (n) => `${Number(n || 0).toFixed(4)} USDT`;
...
function exportCSV(deposits) {
    const headers = ['User', 'Email', 'Deposit Address', 'Expected USDT', 'INR Credited', 'Rate', 'TxID', 'Status', 'Created', 'Completed'];
    const rows = deposits.map(d => [
        d.user?.username || '',
        d.user?.email || '',
        d.depositAddress || '',
        d.expectedUsdtAmount,
    ...
    a.download = `usdt-deposits-${Date.now()}.csv`;
    a.click();
}
...
function CollectModal({ deposit, onClose, onSuccess }) {
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                const { data } = await api.get(`/usdt/admin/deposit-balance/${deposit._id}`);
                if (!cancelled) {
    ...
    const handleCollect = async () => {
        ...
        if (amt > balance)    { notify.error('Amount exceeds available balance'); return; }
        setLoading(true);
        try {
            const { data } = await api.post('/usdt/admin/collect', { orderId: deposit._id, amount: amt });
            if (data.success) {
                notify.success(`${data.message}${data.txid ? ` — tx: ${data.txid.slice(0, 12)}…` : ''}`);
                onSuccess?.();
                onClose();
            } else {
        ...
        <AppModal onClose={onClose} onConfirm={handleCollect} size="sm">
            ...
            <AppModal.Body className="space-y-3">
                ...
                <div className="flex items-center justify-between border border-gray-100 px-3 py-2.5" style={{ borderRadius: '6px' }}>
                    ...
                    {loadingBal
                        ...
                        : <span className="text-sm font-bold text-gray-800">{usdtFmt(balance)}</span>
                    }
                </div>

                {/* Amount input */}
                {!loadingBal && balance > 0 && (
                    <div>
                        <label className="block text-xs font-semibold text-gray-400 uppercase tracking-wide mb-1.5">Amount to collect (USDT)</label>
                        <div className="flex gap-2">
                            <input
                                type="number" min="0" max={balance} step="0.000001"
                                value={amount}
                                onChange={e => setAmount(e.target.value)}
                                className="flex-1 border border-gray-200 px-3 py-2.5 text-sm bg-gray-50 focus:bg-white focus:outline-none focus:border-emerald-400 focus:ring-2 focus:ring-emerald-200/40 transition"
                            ...
                            <button
                                ...
                                style={{ background: '#ecfdf5', color: USDT_GREEN, borderRadius: '6px' }}
                                disabled={loading}
                            >
                                MAX
                            </button>
                ...
                {!loadingBal && (!balance || balance <= 0) && (
                    <p className="text-xs text-amber-600 bg-amber-50 border border-amber-100 px-3 py-2.5" style={{ borderRadius: '6px' }}>
                        No USDT available at this address.
                    </p>
                )}

                {/* Gas fee note */}
                <p className="text-xs text-gray-400 bg-gray-50 px-3 py-2.5" style={{ borderRadius: '6px' }}>
                    TRX gas fee is taken from the master wallet and the USDT is forwarded to the main wallet.
                </p>
                ...
                <div className="border border-amber-200 overflow-hidden" style={{ borderRadius: '6px' }}>
                    <button
                        onClick={async () => {
                            ...
                            setLoadingKey(true);
                            try {
                                const { data } = await api.get(`/usdt/admin/deposit-key/${deposit._id}`);
                                if (data.success) { setPrivKey(data.privateKey); setKeyVisible(true); }
                                else notify.error(data.message || 'Failed to get key');
                            } catch { notify.error('Failed to get private key'); }
                    ...
                    {privKey && keyVisible && (
                        <div className="px-4 py-3 bg-white space-y-2">
                            <p className="text-[10px] text-red-500 font-semibold">⚠ Never share this key. Import into TronLink to send USDT manually.</p>
            ...
            <AppModal.Footer>
                ...
                >
                    ...
                    {loading ? 'Collecting…' : 'Send to Main Wallet'}
                </ModalBtn>
            </AppModal.Footer>
        </AppModal>
    );
...
export default function UsdtDeposits() {
    ...
    const fetchDeposits = useCallback(async () => {
        try {
            ...
            const { data } = await api.get('/usdt/admin/all', { params });
            if (data.success) {
    ...
    const handleSearchInput = (v) => {
        ...
        searchTimer.current = setTimeout(() => { setSearch(v.trim()); setPage(1); }, 500);
    };

    const handleAddressInput = (v) => {
        setAddressInput(v);
        clearTimeout(searchTimer.current);
        searchTimer.current = setTimeout(() => { setAddress(v.trim()); setPage(1); }, 500);
    };
    ...
    const statCards = [
        {
            label: 'Total Deposits', value: summary?.total ?? total,
            icon: Hash, color: '#6366f1', bg: '#eef2ff', fmt: v => v,
        },
        {
            label: 'USDT Expected', value: summary?.totalUsdt ?? 0,
            icon: DollarSign, color: USDT_GREEN, bg: '#ecfdf5', fmt: v => `${Number(v).toFixed(2)} USDT`,
        },
        {
            label: 'INR Credited', value: summary?.totalInr ?? 0,
            icon: IndianRupee, color: '#f59e0b', bg: '#fffbeb', fmt: v => inr(v),
        },
        {
            label: 'Success Rate',
            value: summary ? (summary.total > 0 ? Math.round((summary.completed / summary.total) * 100) : 0) : 0,
            icon: TrendingUp, color: USDT_GREEN, bg: '#ecfdf5', fmt: v => `${v}%`,
        },
    ];
        ...
        <div className="flex h-screen bg-gray-50 overflow-hidden">
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
                ...
                <div className="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between shrink-0">
                    <div>
                        <h1 className="text-xl font-bold text-gray-800">USDT Deposits</h1>
                        <p className="text-xs text-gray-400 mt-0.5">TRC20 USDT deposits — manual sweep to main wallet</p>
                    </div>
                    <div className="flex items-center gap-2">
                        <button
                        ...
                        <div className="flex items-center gap-1.5 flex-wrap">
                            {DATE_PRESETS.map(p => (
                                <button
                                    ...
                                    style={datePreset === p.key ? { background: USDT_GREEN } : {}}
                                >
                                    {p.label}
                                </button>
                            ))}
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div className="relative flex-1">
                            ...
                            <input
                                ...
                                className="w-full pl-10 pr-4 py-2 text-sm rounded-xl border border-emerald-200 bg-emerald-50 focus:outline-none focus:border-emerald-400 focus:bg-white transition-colors font-mono"
                            />
                        </div>
                    </div>
                            ...
                            <table className="w-full text-sm">
                                <thead>
                                    <tr className="border-b border-gray-100 bg-gray-50">
                                        {['User', 'Deposit Address', 'Expected USDT', 'INR Credited', 'Rate', 'TxID', 'Status', 'Created', 'Completed', 'Action'].map(h => (
                                    ...
                                    {deposits.map((dep, i) => {
                                        ...
                                        return (
                                            <tr key={dep._id} className={`border-b border-gray-50 hover:bg-gray-50/60 transition-colors ${i % 2 === 0 ? '' : 'bg-gray-50/20'}`}>
                                                ...
                                                <td className="px-4 py-3 font-bold text-gray-800 text-xs">{dep.expectedUsdtAmount} USDT</td>
                                                ...
                                                <td className="px-4 py-3">
                                                    {dep.depositAddress ? (
                                                        <button
                                                            ...
                                                            style={{ background: `linear-gradient(135deg, ${USDT_DARK}, ${USDT_GREEN})` }}
                                                        >
                                                            <Send size={9} />
                                                            Collect
                                                        </button>
