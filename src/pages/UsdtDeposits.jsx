...
import {
    ...
    Hash, IndianRupee, DollarSign, Key, Eye, EyeOff, Copy, X,
} from 'lucide-react';
...
function exportCSV(deposits) {
    ...
    const rows = deposits.map(d => [
        ...
        d.expectedUsdtAmount, d.status === 'completed' ? d.inrAmount : '',
...
function CollectModal({ deposit, onClose, onSuccess }) {
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                if (!cancelled) { setBalance(data.balance ?? 0); setAmount(String(data.balance ?? '')); }
    ...
    return (
        <AppModal onClose={onClose} onConfirm={handleCollect} size="sm">
            <AppModal.Header icon={<DollarSign size={15} />} title="Collect USDT" subtitle={deposit.user?.username || '—'} onClose={onClose} accent="emerald" />
            <AppModal.Body className="space-y-3">
                ...
                <div className="flex items-center justify-between border border-gray-100 px-3 py-2.5">
                    ...
                    {loadingBal ? <Loader2 size={14} className="animate-spin text-gray-400" />
                        : <span className="text-sm font-bold text-gray-800">{Number(balance || 0).toFixed(4)} USDT</span>}
                </div>
                {!loadingBal && balance > 0 && (
                    <div>
                        ...
                        <div className="flex gap-2">
                            <input type="number" min="0" max={balance} step="0.000001"
                                value={amount} onChange={e => setAmount(e.target.value)}
                                className={inputCls + ' flex-1'} placeholder="0.000000" disabled={loading} />
                ...
                {!loadingBal && (!balance || balance <= 0) && (
                    <p className="text-xs text-amber-600 bg-amber-50 border border-amber-100 px-3 py-2.5">No USDT available at this address.</p>
                )}
                <p className="text-xs text-gray-400 bg-gray-50 px-3 py-2.5">
            ...
            <AppModal.Footer>
                ...
                <ModalBtn variant="emerald" onClick={handleCollect} disabled={loading || loadingBal || !balance || balance <= 0} className="flex items-center gap-1.5">
            ...
            <main className="flex-1 overflow-auto">
                ...
                <header className="bg-white border-b border-gray-200 px-4 md:px-8 py-3 md:py-4 flex items-center justify-between md:sticky md:top-0 z-10">
                    <div className="min-w-0">
                        <h1 className="text-lg md:text-xl font-bold text-gray-900 tracking-tight">USDT Deposits</h1>
                        <p className="hidden md:block text-xs text-gray-400 mt-0.5">TRC20 USDT deposits — manual sweep to main wallet</p>
                    </div>
                ...
                <div className="p-4 md:p-6 lg:p-8 space-y-4 md:space-y-5">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
                        {[
                            ...
                            { label: 'USDT Expected',  value: summary?.totalUsdt ?? 0,      valueColor: '#111827', icon: DollarSign,  iconColor: '#d97706', iconBg: '#fef9c3', fmt: v => `${Number(v).toFixed(2)} USDT` },
                                    ...
                                    {loading ? (
                                        ...
                                        <tr>
                                            <td colSpan={10} className="py-16 text-center">
                                                ...
                                                <p className="text-gray-500 font-medium">No USDT deposits found</p>
